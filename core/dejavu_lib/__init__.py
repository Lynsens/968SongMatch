...
import dejavu_lib.logic.decoder as decoder
from dejavu_lib.base_classes.base_database import get_database
from dejavu_lib.config.settings import (DEFAULT_FS, DEFAULT_OVERLAP_RATIO,
...
from dejavu_lib.logic.fingerprint import fingerprint
...
class Dejavu:
    ...
    def fingerprint_file(self, file_path: str, song_name: str = None) -> None:
        ...
        if song_hash in self.songhashes_set:
            ...
            extracted_song_name, hashes, file_hash = Dejavu._fingerprint_worker(
                (file_path, self.limit)
            )
            # Use provided song_name or fall back to extracted name from file
            if not song_name:
                song_name = extracted_song_name
            sid = self.db.insert_song(song_name, file_hash, len(hashes))
